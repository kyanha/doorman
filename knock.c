...
void usage(char c)
{
   ...
   fprintf (stderr, "  knock [-h] [-v] [-p port] [-f pathname] [-g groupid] \\\n");
   ...
   fprintf (stderr, "    -v             print version number the and "
                     "quit\n");
   fprintf (stderr, "    -f pathname    use this configuration file\n") ;
...
//                 ----------------------
    ...
    while ((c = getopt(argc, argv, "?f:g:hp:r:s:v")) != -1) {

        switch (c) {
            case 'f':
                strncpy (cfg_name, optarg, 500) ;
                break ;
            case 'g':
...
//                 ----------------------
    ...
    if (cfg_name[0] == 0) {
        char profileDir[260] ;
        n = ExpandEnvironmentStrings ("%USERPROFILE%", profileDir, 256) ;
        if (n) {
            snprintf (cfg_name, 500, "%s\\%s", profileDir, KNOCK_CFGFILE_WIN) ;
        } else {
            snprintf (cfg_name, 500, "%s\\%s", "C:", KNOCK_CFGFILE_WIN) ;
        }
    }
    ...
    if (cfg_name[0] == 0) {
        sprintf (cfg_name, "%s/%s", getenv("HOME"), KNOCK_CFGFILE_UNIX);
    }
...
//                 ----------------------------
//                 -  run post-knock command  -
//                 ----------------------------


    if ((command[0] != 0) && (command[0] != 0x7F)) {
